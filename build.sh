...
if [ ! -f "$CAP/public/home.html.ref" ]; then
  node "$S/faithful-home.mjs" --no-scripts --src "https://www.nylpj.com/" --pages "$PAGES" --dir "$CAP"
fi
mkdir -p "$PROJ/public"
...
python3 "$PROJ/scripts/remove-header-background-logo.py" "$PROJ"
...
python3 "$PROJ/scripts/shreveport-conversion-pass.py" "$PROJ"
